...
// the github.com/sunnogo/build tree. It only updates files if they are
...
func main() {
	root, err := build.Import("github.com/sunnogo/build", "", build.FindOnly)
	if err != nil {
		log.Fatalf("failed to find github.com/sunnogo/build root: %v", err)
	}
	err = filepath.Walk(root.Dir, func(path string, fi os.FileInfo, err error) error {
		...
		pkgName := "github.com/sunnogo/build/" + filepath.ToSlash(rest)
